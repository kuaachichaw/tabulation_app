...
export default function AassignSegment() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Combine solo and pair segments into a single array
    ...
    useEffect(() => {
        ...
        const fetchAssignments = async () => {
            try {
                ...
                // Handle response based on segment type
                let assigned = {};
                if (selectedSegment.type === 'solo') {
                    // For solo segments, res.data is an array of judge IDs
                    assigned = res.data.reduce((acc, judgeId) => {
                        acc[judgeId] = true;
                        return acc;
                    }, {});
                } else {
                    // For pair segments, res.data is an array of assignment objects with judge details
                    assigned = res.data.reduce((acc, assignment) => {
                        acc[assignment.judge.id] = true; // Use judge.id to mark as assigned
                        return acc;
                    }, {});
                }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Prepare the payload based on segment type
            const payload = selectedSegment.type === 'solo'
                ? {
                    segment_id: selectedSegmentId, // For solo segments
                    assignments: assignments, // Send the assignments object directly
                }
                : {
                    pair_segment_id: selectedSegmentId, // For pair segments
                    assignments: assignments, // Send the assignments object directly
                };
    ...
    // Log the segments data being passed to SegmentSelector
    // console.log('Segments data passed to SegmentSelector:', segments);
    ...
    Assign {
        selectedSegmentId
            ? segments.find(s => s.id === selectedSegmentId)?.pair_name
                ? `${segments.find(s => s.id === selectedSegmentId)?.pair_name}`
