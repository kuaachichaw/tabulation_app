...
const ScoringCandidates = ({ 
  ...
  const [displayMode, setDisplayMode] = useState(2); // Default to Both
  const [loadingDisplayMode, setLoadingDisplayMode] = useState(true);

  // Fetch display mode from backend
  useEffect(() => {
    const fetchDisplayMode = async () => {
      try {
        const response = await axios.get('/display/get');
        setDisplayMode(response.data.Display_Scoring ?? 2); // Fallback to 2 if null
      } catch (error) {
        console.error("Error fetching display mode:", error);
        setDisplayMode(2); // Fallback to Both on error
      } finally {
        setLoadingDisplayMode(false);
      }
    };
  ...
  // Helper function to check if candidate is selected
  ...
  if (loadingDisplayMode) {
    return <div className="flex justify-center p-8">Loading display settings...</div>;
  }

  return (
    <div className="space-y-6">
      {/* Individual Candidates Section - Only shown when NOT in Pair-only mode */}
      {displayMode !== 1 && (
        <motion.div
        ...
        >
          {/* Only show header if in Both mode */}
          {displayMode === 2 && (
      ...
      {/* Pair Candidates Section - Only shown when NOT in Solo-only mode */}
      {displayMode !== 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3, delay: displayMode === 2 ? 0.1 : 0 }}
        >
          {/* Only show header if in Both mode */}
          {displayMode === 2 && (
